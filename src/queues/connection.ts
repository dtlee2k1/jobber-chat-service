...
const logger = winstonLogger(`${envConfig.ELASTIC_SEARCH_URL}`, 'chatQueueConnection', 'debug');
...
export async function createConnection() {
  try {
    ...
    logger.log({ level: 'error', message: `ChatService createConnection() method error: ${error}` });
    return undefined;
  }
}
